...
class Header extends Component
{
    ...
    {
        // на этапе инициализации мы получаем от НЛО-пользователя содержимое его локального хранилища браузера на предмет наличия ранее оформленных id-заказов и если такие были, 
        // то смотрим, остались ли они в БД (я их мог "руками" удалить или они были удалены системой из-за сомнительности таких заказов, могли быть оформлены "от балды", типа посмотреть, ф что будет, если...):
        $idsOrdersInUserLocalStorage = "";
        $idsOrdersInUserLocalStorageToDel = "";
        ...
        if($this->ordersIds) {
            ...
            // перезапишем обратно в json (для передачи в JS) массив актуальных id-заказов чтобы "почистить" локальное хранилище браузера НЛО-пользователя:
            $idsOrdersInUserLocalStorage = json_encode($ordersIdsArr);
            $idsOrdersInUserLocalStorageToDel = json_encode($idsOrdersInUserLocalStorageToDel);
            //dump($idsOrdersInUserLocalStorageToDel);
        }
        ...
        // dd($idsOrdersInUserLocalStorageToDel);
        ...
        return view('components.header', [
            ...
            'idsOrdersInUserLocalStorage' => $idsOrdersInUserLocalStorage,
            'idsOrdersInUserLocalStorageToDel' => $idsOrdersInUserLocalStorageToDel,
        ]);
    }
