...
class NewOrderForCustomer extends Mailable
{
    ...
    {
        return new Envelope(
            subject: 'Мой заказ флорбольной экипировки в магазине UnihocZoneRussia',
        );
    }
    ...
    public function content()
    {
        ...
        foreach($productsArr as $product) {
            $productName = Product::find($product[0])->title;           // С помощью специального метода find можно получить запись по ее id
            ...
            $dataAboutGoodsForMailBody .= '<tr><td style="text-align: center;">' . $i . '</td><td>' . $productName . '</td><td style="text-align: center;">' . $product[1] . '
        ...
        return new Content(
            view: 'emails.orders.neworderinfoforcustomer',
            with: [
