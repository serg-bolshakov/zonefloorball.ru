// resources/js/Components/Admin/Documents/AssemblyProductSelector.tsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';

interface IAssemblyProductSelectorProps {
    onProductSelect: (product: any) => void;
}

export const AssemblyProductSelector: React.FC<IAssemblyProductSelectorProps> = ({ onProductSelect }) => {
    ...
    // Загружаем категории при монтировании, используем ту, что для собираемых товаров
    useEffect(() => {
        loadCategoriesWithAssembled();
    }, []);
    ...
    const loadCategoriesWithAssembled = async () => {
        try {
            const response = await axios.get('/api/categories-with-assembly-products');
            console.log('loadCategoriesWithAssembled', response.data);
    ...
    const loadCategoryProducts = async (categoryId: number) => {
        try {
            const response = await axios.get(`/api/categories/${categoryId}/assembly-products`);
    ...
    const searchProducts = async (term: string) => {
        ...
        try {
            const response = await axios.get('/api/products/assembly-search', {
                params: { q: term }
            });
